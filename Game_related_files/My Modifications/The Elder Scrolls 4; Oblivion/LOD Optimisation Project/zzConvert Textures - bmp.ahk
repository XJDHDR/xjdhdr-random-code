Loop, Files, %A_ScriptDir%\aaS - RAEVWD textures\*, FR
{
	...
	StringReplace, ConvertedTextureDir, A_LoopFileDir, aaS - RAEVWD, aD - Converted - bmp,
	...
	{
		RunWait, %ComSpec% /c ""%A_ScriptDir%\ImageMagick\magick.exe" convert "%SourceTexture%" "%ConvertedTextureDir%\%NewFileName%" 1>>"%A_ScriptDir%\out-bmp.txt" 2>>"%A_ScriptDir%\errors-bmp.txt"", , Min,
		Continue
	}
	...
	RunWait, %ComSpec% /c ""%A_ScriptDir%\ImageMagick\magick.exe" convert "%SourceTexture%" -colorspace RGB -filter point -resize 50`%`% -colorspace sRGB "%ConvertedTextureDir%\%NewFileName%" 1>>"%A_ScriptDir%\out-bmp.txt" 2>>"%A_ScriptDir%\errors-bmp.txt"", , Min,
}
